...
class Config:
    ...
    KEY_VAULT_SECRET_MAP = [
        ...
        # Tool Integration
        ("GRAPH_TENANT_ID", "microsoft-graph-secrets.graph_tenant_id"),
        ("GRAPH_CLIENT_ID", "microsoft-graph-secrets.graph_client_id"),
        ("GRAPH_CLIENT_SECRET", "microsoft-graph-secrets.graph_client_secret"),
        ("ERP_API_KEY", "erp-secrets.erp_api_key"),
        ("COMPLIANCE_API_KEY", "compliance-secrets.compliance_api_key"),
    ]
...
def _initialize_config():
    ...
    # All config variables needed by agent and referenced modules
    CONFIG_VARIABLES = [
        # General
        "ENVIRONMENT",
        # LLM / Model
    ...
    # Add Service Principal variables if not using DefaultAzureCredential
    if not AZURE_USE_DEFAULT_CREDENTIAL:
        CONFIG_VARIABLES += AZURE_SP_VARS
    ...
    for var_name in CONFIG_VARIABLES:
        ...
        # Special: LLM_MODELS can be a JSON string, parse if present
        if var_name == "LLM_MODELS" and value:
            ...
            try:
                ...
                value = []
        # Special: CONTENT_SAFETY_ENABLED (bool)
        elif var_name == "CONTENT_SAFETY_ENABLED":
            value = (str(value).lower() in ("true", "1", "yes", "on")) if value is not None else False
        # Special: CONTENT_SAFETY_SEVERITY_THRESHOLD (int)
        elif var_name == "CONTENT_SAFETY_SEVERITY_THRESHOLD":
            try:
                value = int(value)
            except (TypeError, ValueError):
                value = 2
        # Special: OBS_AZURE_SQL_TRUST_SERVER_CERTIFICATE (default "yes")
        ...
        if value is None or value == "":
            ...
            # Set to empty string or None as appropriate
